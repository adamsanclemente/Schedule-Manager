...
export async function scheduleJobEvents(
	workerId: string,
	workerName: string,
	jobId: string,
	jobName: string,
...
async function getExistingWorkerEvents(workerId: string, date: Date): Promise<Event[]> {
	...
	const filtered = workerEvents.filter((event: { start: Date }) => {
		return event.start.toDateString() === date.toDateString();
	});

	return filtered.map(
		(event: { employeeId: string; jobId: string; start: Date; end: Date; title: string }) => {
			return {
