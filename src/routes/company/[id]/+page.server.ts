...
export const load = (async ({ params }) => {
	...
	let monthEvents = events.filter((event: any, index: number, self: any[]) => {
		return (
			index ===
			self.findIndex(
				(e: any) =>
	...
	monthEvents = monthEvents.map((event: any) => {
		return {
			...event,
			allDay: true
		};
...
export const actions = {
	...
	createEvent: async (event) => {
		...
		const eventsToCreate = await scheduleJobEvents(
			worker.id,
			`${worker.firstName} ${worker.lastName}`,
			job.id,
			job.title,
			job.hours,
			new Date(form.data.startDate),
			worker.maxHours
		);
		...
		for (const event of eventsToCreate) {
			...
			const startInEastern = new Date(
				event.start.getTime() - timezoneOffset * 60000 + 4 * 60 * 60 * 1000
			);
			const endInEastern = new Date(
				event.end.getTime() - timezoneOffset * 60000 + 4 * 60 * 60 * 1000
			);
